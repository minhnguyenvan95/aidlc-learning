...
| FR-21 | Câu hỏi xác minh hiển thị random tối đa 3 câu (từ pool lớn hơn), mỗi câu có options trắc nghiệm (A, B, C...) + option cuối "Khác" cho điền tự do | Cao |
...
| FR-20 | Sau khi chọn option, hiển thị nút "Tiếp tục →" inline ngay dưới kết quả để người dùng không cần cuộn lên header | Cao |
| FR-22 | Sau khi click phê duyệt, giả lập AI đang xử lý (loading spinner + "Đang xử lý..." 1-2 giây) trước khi hiện kết quả | Cao |
| FR-23 | Phần output kết quả (decision-result + io-summary) cũng chạy chữ typewriter khi xuất hiện | Cao |
...
| NFR-04 | Animation typewriter chạy từng ký tự text (không chạy nguyên block UI). Block chưa đến lượt thì ẩn hoàn toàn, chỉ hiện khi block trước hoàn tất. |
| NFR-05 | Responsive — hamburger button mở/đóng sidebar trên mobile (≤768px) |
